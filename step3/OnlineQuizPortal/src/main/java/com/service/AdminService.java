...
public class AdminService {
	...
	public String updateQuestion(Question qn) {
		Question a = adminDao.getQestionDetails(qn.getQid());
		if(a==null) {
			return "Question details not present, cannot be updated";
		}
		else if(adminDao.updateQuestion(qn)>0) {
				return "Question Updated";
			}else {
				return "Question  Not Updated";
			}
	}
	public String updateQuiz(Quiz qz) {
		Quiz a = adminDao.getQuizDetails(qz.getQzid());
		if(a==null) {
			return "Quiz details not present, cannot be updated";
		}
		else if(adminDao.updateQuiz(qz)>0) {
				return "Quiz Updated";
			}else {
				return "Quiz  Not Updated";
			}
	}
